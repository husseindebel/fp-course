...
headOr ::
  ...
  -> List a
  -> a
headOr a xs = foldRight (\b _ -> b) a xs
...
length ::
  List a
  -> Int
length x = foldRight (\_ b -> b + 1) 0 x
...
map ::
  ...
  -> List a
  -> List b
map f xs = foldRight (\x y -> f x :. y) Nil xs
...
seqOptional x = foldRight (twiceOptional (:.)) (Full Nil) x
